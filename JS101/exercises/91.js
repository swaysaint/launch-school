...
// through manually. Consider a MULT operation in a stack-and-register
// language. It removes the value from the stack, multiplies the removed stack
// value with the register value, then stores the result back in the register.
// For example, if we start with a stack of [3, 6, 4] (where 4 is the topmost
// item in the stack), and a register value of 7, the MULT operation mutates the
// stack to [3, 6] (the 4 is removed), and the result of the multiplication, 28,
// is left in the register. If we do another MULT at this point, the stack is
...
//
// Initialize the stack and register to the values of [] and 0, respectively.

const stack = [];
let register = 0;

function minilang(str) {
  const opList = str.split(' ');
  for (let idx = 0; idx < opList.length; idx++) {
    if (parseInt(opList[idx])) {
      register = Number(opList[idx]);
    } else {
      performOperation(opList[idx]);
    }
  }
}
