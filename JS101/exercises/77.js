...
// false based on whether an inventory item is available. As before, the
...
function isItemAvailable(transId, transactions) {
  return transactionsFor(transId, transactions).reduce((count, transaction) => {
...
function transactionsFor(transId, transactionList) {
  return transactionList.filter(transaction => transaction.id === transId);
}
